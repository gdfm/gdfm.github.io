---
layout: page
title: "About me"
permalink: /
---

<img src="/assets/media/2009/03/bw.jpg" alt="Just a picture of me" style="float:right;margin:0 0 1.5rem 2rem;max-width:280px;width:45%;">

Hello, world!

My name is *Gianmarco De Francisci Morales* and I work as Principal Researcher at [CENTAI](https://centai.eu), where I lead the Social Algorithmics Team (SALT). In the past I worked as Senior Research Scientist at [ISI Foundation](https://www.isi.it/en/home), Scientist at [QCRI](http://www.qcri.com), as Visiting Scientist at [Aalto University](http://www.aalto.fi/en), as Research Scientist at [Yahoo Labs](http://labs.yahoo.com), and as Research Associate at the [HPC Lab](http://hpc.isti.cnr.it) of [ISTI-CNR](http://www.isti.cnr.it).

I hold a Ph.D. in [CSE](http://www.imtlucca.it/phd_programs/computer_science_engineering/index.php) (Computer Science and Engineering) from [IMT Lucca](http://www.imtlucca.it), and Master's and Bachelor's degrees in Computer Engineering from [University of Catania](http://www2.unict.it/en/).

My main research interest is to understand how people form their opinion.
For instance, understanding how controversial topics are discussed online, whether polarization is created or amplified by social media, and applying computational methods to understand online discussions.
In the past, I also worked on scalable data mining (especially graphs and streams) and data-intensive scalable computing. Hence, I am a heavy user of the Apache Hadoop ecosystem. I am a committer for [Pig](http://pig.apache.org), and a contributor for [Hadoop](http://hadoop.apache.org), [Giraph](http://giraph.apache.org), [Storm](http://storm.apache.org), [Kafka](http://kafka.apache.org), [S4](http://incubator.apache.org/s4), and [Grafos](http://grafos.ml). I am also a PPMC member and one of the original creators of [SAMOA](http://samoa.incubator.apache.org), a platform for mining big data streams.

My Ph.D. thesis explored the issues around massive-scale Web mining using DISC.
[“Big Data and the Web: Algorithms for Data Intensive Scalable Computing”](/assets/media/2012/03/big-data-and-the-web.pdf).
Here also my [PhD proposal](/assets/media/2010/02/thesis-proposal.pdf) for the (very) interested reader.

My master's thesis dealt with parallel multi-objective genetic algorithms for optimization of embedded systems. The software I contributed to is open source: [Epic-Explorer](http://sourceforge.net/projects/epic-explorer/)

In my private life I love reading, cooking, traveling, and learning new languages.

Feel free to drop me an email: gdfm@acm.org

All the content published on this website is distributed under an Attribution-ShareAlike 4.0 Unported Creative Commons license [(CC BY-SA 4.0)](http://creativecommons.org/licenses/by-sa/4.0), unless otherwise specified.

## Links

* [My Bluesky](https://bsky.app/profile/gdfm.bsky.social)
* [My Curriculum Vitae](https://melmeric.wordpress.com/wp-content/uploads/2025/02/cv-gdfm-20250209.pdf)
* [My GitHub](http://github.com/gdfm)
* [My GPG key](http://pgp.mit.edu/pks/lookup?op=get&search=0x6D26F8392A7AAA60)
* [My LinkedIn](http://www.linkedin.com/in/gianmarcodfm)
* [My Speaker Deck](https://speakerdeck.com/u/gdfm)
* [My Zotero](https://www.zotero.org/gdfm)
